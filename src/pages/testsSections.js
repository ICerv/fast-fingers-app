...
]},
{
 ...
 exercises: 
 [
 'Flexbox je moderní CSS layoutový modul, který poskytuje flexibilní způsob uspořádání prvků uvnitř kontejneru. Flexbox je navržen tak, aby řešil problémy s umisťováním a zarovnáváním prvků v rámci responzivních webových designů. Flexbox funguje pomocí kontejneru a prvků uvnitř něj. Kontejner se označuje jako flexový kontejner (flex container) a prvky uvnitř jsou flexové prvky (flex items). Flexbox poskytuje různé vlastnosti a pravidla, která umožňují ovlivňovat chování a rozložení těchto prvků. Některé z hlavních vlastností Flexboxu zahrnují:- Možnost definovat orientaci prvků pomocí vlastnosti "flex-direction", která určuje, zda se prvky řadí horizontálně nebo vertikálně.Flexibilní přizpůsobování velikosti prvků pomocí vlastnosti "flex-grow", "flex-shrink" a "flex-basis", které určují, jak se prvky rozdělují v prostoru.Možnost zarovnávat prvky v rámci kontejneru pomocí vlastností "justify-content" (horizontální zarovnání) a "align-items" (vertikální zarovnání).Možnost měnit pořadí prvků pomocí vlastnosti "order", která určuje, v jakém pořadí se prvky zobrazují.Flexbox je výkonný nástroj pro vytváření responzivních a flexibilních rozložení prvků ve webových designech. Jeho jednoduchá syntaxe a podpora ve většině moderních prohlížečů ho činí oblíbeným nástrojem pro vývojáře.'
 ],
 },
 { 
 id: 2,
 exercises: 
 [
 'CSS Grid je mocný layoutový modul v CSS, který umožňuje vytvářet kompletní mřížková rozložení webových stránek. S CSS Gridem můžete snadno a flexibilně rozdělit obsah na sloupce a řádky, které se přizpůsobují velikosti okna prohlížeče.Hlavní prvky CSS Gridu jsou mřížkový kontejner (grid container) a mřížkové prvky (grid items). Mřížkový kontejner definuje mřížku pomocí vlastnosti "display: grid" a určuje počet sloupců, řádků a jejich velikosti. Mřížkové prvky jsou umístěny uvnitř kontejneru a mohou být přiděleny do jednoho nebo více sloupců a řádků.CSS Grid poskytuje širokou škálu vlastností, které umožňují ovlivňovat rozložení prvků:"grid-template-columns" a "grid-template-rows" určují počet a velikost sloupců a řádků."grid-gap" definuje mezery mezi sloupci a řádky."grid-column" a "grid-row" určují, do kterých sloupců a řádků patří daný prvek."grid-area" umožňuje pojmenovat oblasti mřížky a přiřadit prvky do těchto oblastí.Díky CSS Gridu můžete snadno vytvářet složité a responzivní mřížkové rozložení, které se automaticky přizpůsobuje různým zařízením a velikostem obrazovek. CSS Grid je podporován ve většině moderních prohlížečů a poskytuje výkonný a efektivní způsob, jak spravovat rozložení prvků ve webovém designu.'
 ],
},
 { 
 id: 3,
 exercises: 
 [
 'Responzivní design je přístup k vývoji webových stránek, který se zaměřuje na tvorbu rozložení a designu, které se automaticky přizpůsobuje různým zařízením a velikostem obrazovek. Cílem responzivního designu je zajistit optimální uživatelský zážitek a čitelnost obsahu na mobilních zařízeních, tabletech a desktopových počítačích.Responzivní design se opírá o použití flexibilních a fluidních jednotek pro rozmístění prvků, jako jsou procenta, em nebo viewport units (vw, vh). Kombinace CSS media queries umožňuje nastavovat různá rozložení, styly a vlastnosti na základě velikosti obrazovky. Tím lze například skrýt nebo přesunout určité prvky na menších obrazovkách, upravit velikost textu nebo změnit uspořádání sloupců.Díky responzivnímu designu se stránky automaticky přizpůsobují velikosti obrazovky a zajišťují, že obsah zůstává čitelný a dobře strukturovaný. To přináší výhody v oblastech uživatelského zážitku, konverzí, SEO (optimalizace pro vyhledávače) a snadné správy obsahu. Uživatelé mají možnost pohodlně procházet stránky bez nutnosti přibližování nebo posouvání a mají k dispozici jednotný a konzistentní zážitek na různých zařízeních.'
 ],
},
 { 
 id: 4,
 exercises: 
 [
 'Webpack je populární nástroj pro správu balíčků a sestavování kódu ve webových aplikacích. Jeho hlavním účelem je zpracování modulárního JavaScriptu, ale je také schopný pracovat s dalšími typy souborů, jako jsou styly, obrázky a další prostřednictvím tzv. "loaderů".Webpack funguje na principu vytváření závislostního grafu všech souborů v projektu a jejich transformace do podoby, kterou lze snadno nasadit v prohlížeči. Pomocí konfiguračního souboru (typicky webpack.config.js) lze definovat různá pravidla a pluginy, které upravují chování a výstupní balíčky Webpacku.Mezi hlavní vlastnosti a funkce Webpacku patří:Modulární sestavování: Webpack umožňuje rozdělit kód do modulů a spravovat jejich závislosti. Tím umožňuje organizovat a sdílet kód mezi různými soubory a balíčky.Loader systém: Webpack využívá loadery pro zpracování různých typů souborů. Loadery jsou řetězené transformace, které umožňují převádět soubory na moduly, které jsou srozumitelné pro prohlížeč. Můžete použít loadery například pro kompilaci a optimalizaci JavaScriptu (Babel), zpracování stylů (CSS, SASS) nebo optimalizaci obrázků.Code Splitting: Webpack podporuje rozdělení kódu do menších částí nazývaných "chunks". Tím umožňuje načítat pouze potřebný kód pro danou stránku nebo funkci, což vede k rychlejšímu načítání a optimalizaci výkonu aplikace.Horká aktualizace (Hot Module Replacement - HMR): Tato funkce umožňuje okamžitou aktualizaci kódu v prohlížeči bez nutnosti znovu načítat celou stránku. HMR zajišťuje rychlé iterace během vývoje, kdy můžete okamžitě vidět změny ve své aplikaci.Optimalizace a minifikace: Webpack nabízí různé pluginy pro optimalizaci a minifikaci kódu. Tyto pluginy snižují velikost a zvýšení výkonu vaší aplikace tím, že odstraňují nepotřebný kód, komprimují soubory a provádějí další optimalizační postupy.'
 ],
},
 { 
 id: 5,
 exercises: 
 [
 'CRUD je základní sada operací, které se používají pro manipulaci s daty v aplikacích nebo databázových systémech. Každá CRUD operace zajišťuje určitý typ interakce s daty a umožňuje vývojářům provádět různé akce nad daty, jako je vytváření, čtení, aktualizace a mazání.Zde je podrobnější výklad jednotlivých CRUD operací:1. Create (Vytvoření): Create operace slouží k vytvoření nových datových záznamů nebo entit. Při vytváření nového záznamu se specifikuje struktura dat a přiřazují se jim hodnoty. Například v případě relační databáze by se jednalo o vložení nového řádku do tabulky.2. Read (Čtení): Read operace umožňuje získávání existujících dat nebo záznamů. Tímto způsobem můžete získat potřebná data z databáze nebo jiného úložiště a použít je v aplikaci. Příklady zahrnují čtení řádků z databázové tabulky nebo načítání souborů z disku.3. Update (Aktualizace): Update operace slouží k modifikaci existujících dat nebo záznamů. Tím umožňuje vývojářům aktualizovat informace uložené v databázi či jiném úložišti. Například můžete upravit určité pole v řádku tabulky nebo změnit hodnotu v souboru.4. Delete (Mazání): Delete operace slouží k odstranění dat nebo záznamů. Tímto způsobem můžete odstranit nepotřebné nebo zastaralé informace z databáze nebo jiného úložiště. Příkladem je mazání řádků z tabulky nebo odstranění souborů ze systému.CRUD operace jsou základem pro vytváření a správu dat v aplikacích. Použití těchto operací umožňuje vývojářům interagovat s daty, udržovat je aktuální, provádět změny a zajistit konzistenci datového modelu. CRUD operace jsou také často používány při práci s API, kde se pomocí HTTP metod (POST, GET, PUT, DELETE) realizují odpovídající CRUD operace.'
 ],
},
 { 
 id: 6,
 exercises: 
 [
 'API (Application Programming Interface) je sada definic, protokolů a nástrojů, které umožňují komunikaci a interakci mezi různými softwarovými komponentami. API poskytuje specifikace, jaké operace a funkce jsou k dispozici a jak je použít.API slouží jako rozhraní, které umožňuje komunikaci mezi dvěma nebo více softwarovými systémy, aplikacemi nebo službami. Poskytuje jednotné a standardizované rozhraní, které umožňuje přístup a výměnu dat a funkcionalit mezi různými komponentami.Existuje několik typů API, včetně:1. Web API: Web API je specifickým typem API, které umožňuje komunikaci mezi webovými aplikacemi. Web API je obvykle založeno na protokolu HTTP a využívá URL adresy pro definování konkrétních zdrojů nebo operací, ke kterým mohou klientské aplikace přistupovat.2. REST API: REST (Representational State Transfer) je architektonický styl pro návrh API. REST API využívá standardních HTTP metod (GET, POST, PUT, DELETE) pro manipulaci s daty a komunikaci mezi klientem a serverem. REST API se často používá v webových aplikacích a poskytuje jednoduché a rozšiřitelné rozhraní pro přístup k datům.3. SOAP API: SOAP (Simple Object Access Protocol) je protokol pro výměnu strukturovaných zpráv mezi webovými službami. SOAP API je založeno na XML a poskytuje robustní a formální rozhraní pro komunikaci mezi aplikacemi, které mohou být napsány v různých jazycích a provozovány na různých platformách.API mohou poskytovat různé typy funkcionalit, jako je získávání dat (čtení), vytváření nových dat (vytvoření), aktualizace dat (aktualizace) a mazání dat (mazání). Díky API mohou aplikace komunikovat mezi sebou, sdílet data, využívat služby třetích stran a rozšiřovat svou funkcionalitu pomocí externích zdroj'
 ],
},
 { 
 id: 7,
 exercises: 
 [
 'Supabase je moderní open-source platforma, která nabízí kompletní sadu nástrojů pro vývoj a správu webových aplikací a backendů. Jejím hlavním cílem je poskytnout vývojářům jednoduchý a efektivní způsob, jak stavět a spravovat své aplikace s využitím databází a autentizace.Jedním z hlavních prvků Supabase je jejich plně spravovaná PostgreSQL databáze v cloudu. S využitím tohoto databázového řešení mohou vývojáři rychle vytvářet, nasazovat a škálovat své aplikace. Díky použití PostgreSQL mohou vývojáři využít všechny pokročilé funkce a dotazovací možnosti, které tato databáze nabízí.Supabase také poskytuje autentizační službu, která umožňuje vývojářům snadno implementovat přihlašování a registraci uživatelů do svých aplikací. S využitím různých poskytovatelů identity, jako je Google, Facebook nebo GitHub, mohou uživatelé pohodlně přistupovat ke svým účtům.Dalším důležitým prvkem Supabase je serverless funkce. S tímto nástrojem mohou vývojáři vytvářet a nasazovat vlastní backendové funkce bez nutnosti spravovat vlastní serverovou infrastrukturu. Tímto způsobem mohou snadno přidávat funkcionalitu do svých aplikací bez složité konfigurace a nasazení.' 
 ],
},
 { 
 id: 8,
