    ...
    return (
        <div className='results__container'>
            <div className='results__container--section'>
                ...
                <p>Počet provedených chyb: 
                    <span className='results__container--value'> {localStorage.getItem('errorCount')} </span> 
                    a přesnost: 
                    <span className='results__container--value'> {localStorage.getItem('accuracy')}%.</span>
                </p>
                ...
                <div className='results__container--exercises-progress'>
                    <div className='progress__info'>
                        <div className='progress__title' >Průběh cvičení</div>
                        <div className='progress__amount'>{Number(localStorage.getItem('exerciseId')) + 1}/{lesson.exercises.length} dokončeno</div>
                    </div>
                    <div className='results__container--exercises-list'>
                        {lesson.exercises.map((exercise, index) =>  
                            <div 
                                key={index} 
                                className={`results__container--exercise-item ${index <= Number(localStorage.getItem('exerciseId')) ? 'active' : ''}`} 
                                onClick={getExercise}
                            > 
                                {index + 1}
                            </div>
